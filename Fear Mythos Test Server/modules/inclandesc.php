...
function inclandesc_getmoduleinfo(){
	$info = array(
		...
		"prefs-clans"=>array(
			"Internal Clan Description,title",
...
function inclandesc_dohook($hookname,$args){
	global $session;
	$clandesc = get_module_objpref("clans",$session['user']['clanid'],"intclandesc");
		...
		case "header-clan":
			if (httpget('op') == ""){
				if ($session['user']['clanrank'] > CLAN_APPLICANT 
					...
					addnav("Clan Options");
					addnav("Change Internal Description","runmodule.php?module=inclandesc&op=enter");
				}
				if ($clandesc <> ""){
					$sql = "SELECT name FROM ".db_prefix("accounts")." WHERE acctid=$id";
					$res = db_query($sql);
					$row = db_fetch_assoc($res);
					output("`b`&Internal Clan Description`b: `#by %s`0`n",$row['name']);
...
function inclandesc_run(){
	...
	switch ($op){
		case "enter":
			if ($newdesc == ""){
				output("`#An internal clan description is like that of a Clan MotD, but it is meant to be more static, such as the Clan Description.");
				output("The different between internal and external, is that the internal shall only be viewed by those in the clan.`n`n");
				...
				output("`#Here is your new Internal Clan Description:`0`n`n");
