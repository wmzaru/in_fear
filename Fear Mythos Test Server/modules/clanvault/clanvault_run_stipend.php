			...
			switch ($action) {
				case "members":
					if (get_module_pref("stipends")>0) {
						output("Stipend a clan member`n`n");
					...
					if ($stipendflag) {
						...
						while ($row = db_fetch_assoc($result)) {
							if (get_module_pref('showNot','clanvault',$row['acctid'])==1) {
								...
								$msg = array("I have rewarded a member %s for being an exemplary member of our clan. The reward was %s %s.`n`n%s of the clan`n%s", $member, $amount, $type, $ranks[$session['user']['clanrank']], $session['user']['name']);
