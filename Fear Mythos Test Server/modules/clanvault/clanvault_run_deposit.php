		...
		switch ($type) {
			case "gold":
				if ($session['user']['gold'] < $amount || ($gold + $amount) > $MAXAMOUNTOFGOLD) {
					if (isset($_POST['donate']) && $_POST['donate'] == 1) {
						output("You don't have enough gold to donate or there's no room for more gold in your clan's vault.`n");
					} else {
						output("You don't have enough gold to deposit or there's no room for more gold in your clan's vault.`n");
					}
					...
					if ($amount>=0) {
						...
						if ($amount==0) {
							if (($gold + $session['user']['gold']) <= $MAXAMOUNTOFGOLD) {
								...
								output("There's no room for more gold in your clan's vault.`n");
								$checkdeposit = false;
							}
						} else {
			...
			case "gems":
				if ($session['user']['gems'] < $amount || ($gems + $amount) > $MAXAMOUNTOFGEMS) {
					if (isset($_POST['donate']) && $_POST['donate'] == 1) {
						output("You don't have enough gems to donate or there's no room for more gems in your clan's vault.`n");
					} else {
						output("You don't have enough gems to deposit or there's no room for more gems in your clan's vault.`n");
					}
				}elseif(!is_numeric($amount)) {
					output("That is not a number.`n");
				} else {
		...
		if (isset($_POST['donate']) && $_POST['donate'] == 1 && $checkdeposit) {
			...
			while ($row = db_fetch_assoc($result)) {
				if (get_module_pref('showNot','clanvault',$row['acctid'])==1) {
					...
					$msg = array("Here's a little donation for our clan, %s %s `n`n%s", $deposit, $type, $session['user']['name']);
