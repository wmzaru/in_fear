...
if ($timeold>$time && $op<>"enter"){
	output("You have been killed.  This has cost you 1 clan point for your clan`n");
	...
	if ($clanid==0){
		output("Sorry, you must be a member of a clan to explore the Pyramid");
		output_notl("`n");
	}else{
		output("You duck through the entrance to the Forbidden Pyramid, only to find there also appears to be two smaller pyramids, alongside it.  Some strange inscriptions appear next to the entrance.");//, which of the three will you end up in?");
		output_notl("`n`n");
		output("You see two entrances, one to your left, and one to your right.. The choice is yours.");
		output_notl("`n`n`c");
		output("`2Beware Ye Who Enters here,`n`nOnly those who know no fear,");
		output_notl("`n`n");
		output("May tread this hallowed ground,");
		output_notl("`n`n");
		output("Bravery and Courage we will reward,");
		output_notl("`n`n");
		output("Dangers soon to unfold,");
		output_notl("`n`n");
		output("Hidden within these walls.");
		output_notl("`n`n`n`n");
		output("Do ye Enter, or do ye Leave?");
		output_notl("`n`n");
		output("The choice be yours,");
		output_notl("`n`n");
		output("To earn reknown, or flee in fear,");
		output_notl("`n`n");
		output("Those who flee,");
		output_notl("`n`n");
		output("Cannot return,");
		output_notl("`n`n");
		output("Until another time decreed");
		output_notl("`c");
		switch (e_rand(1,3)){
			case 1:
			addnav("Tzeltalchs Pyramid","runmodule.php?module=clanpyramid&op=one");
			addnav("Chexralmins Pyramid","runmodule.php?module=clanpyramid&op=two");
			break;
			case 2:
			addnav("Tzeltalchs Pyramid","runmodule.php?module=clanpyramid&op=one");
			addnav("Prazlynx Pyramid","runmodule.php?module=clanpyramid&op=three");
			break;
			case 3:
			addnav("Chexralmins Pyramid","runmodule.php?module=clanpyramid&op=two");
			addnav("Prazlynx Pyramid","runmodule.php?module=clanpyramid&op=three");
...
if ($op=="one"){
	page_header("Tzeltalchs Pyramid");
	output("Pulling back the vine enshrouded cover stone to the pyramids entrance, you step inside, it takes your eye's a moment to adjust to the dim interior.  You realise in this gloom you can only see a few paces in front and behind you, you wish you had some form of illumination, but it appears this is not to be.");
	output_notl("`n`n");
	if ($owned1==$clanid){
		output("You may enter and defend");
		addnav("Enter Pyramid","runmodule.php?module=clanpyramid&op=defend&p=1");
	...
	addnav("Warriors in Pyramid","runmodule.php?module=clanpyramid&op=listwarriors&p=1");
	villagenav();
	page_footer();
}
if ($op=="listwarriors"){
	...
	if ($square==901){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=one");
	}elseif ($square==222){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=move&move=return&wall=222&p=1");
	}elseif($square==1300){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=two");
	}elseif($square==1095){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=move&move=return&wall=1095&p=2");
	}elseif($square==2157){
		addnav("Back To Pyramid","runmodule.php?module=clanpyramid&op=move&move=return&wall=2157&p=3");
	}elseif($square=="P3"){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=move&move=entry&p=3");
	}elseif($square=="3001"){
		addnav("Back to Pyramid","runmodule.php?module=clanpyramid&op=three&p=3");
	}
}
if ($op=="two"){
	...
	if ($owned2==$clanid){
		output("You may enter and defend");
		addnav("Enter Pyramid","runmodule.php?module=clanpyramid&op=defend&p=2");
...
if ($op=="three"){
	if ($owned3==$clanid){
		output("You may enter and defend");
		addnav("Enter Pyramid","runmodule.php?module=clanpyramid&op=defend&p=3");
		...
		output_notl("`n`n");
		output("Before you have time to wonder what they are doing this far from the Pyramid itself, they are upon you");
		addnav("What Will You Do?");
...
if ($op=="giveup"){
	page_header("Leave the Pyramid");
	output("You have chosen to leave, you will not be able to return to any pyramid for 10 minutes");
...
if ($op=="attack"){
	...
	if ($fightnum==1){
		...
		$badguy = array(
			...
			"startmsg"=>"`^Your clans strength comes to your aid!`n",
			"name"=>"`4Clan Attack",
			"rounds"=>20,
			...
			"effectmsg"=>"`RThe Strength of Your Clan empowers you. You inflict {damage} damage",
	...
	if ($battle){
		...
		if ($victory){
			...
			if ($fightnum==1){
				...
				output_notl("`n`n");
				output("You continue on further down the path, knowing you shall reach the Pyramid soon.  Also knowing, you have reached the point of no return, after this there is no turning back.");
				addnav("Continue On");
				set_module_pref("square","P3");
				addnav("Pyramid Entrance","runmodule.php?module=clanpyramid&op=move&move=entry&p=3");
				addnav("Warriors List","runmodule.php?module=clanpyramid&op=listwarriors&p=3");
				villagenav();
			}elseif ($fightnum==2){
				output_notl("`n");
			...
			if ($fightnum==1){
				...
				addnews("`GThe body of %s `Gwas seen being thrown from a Pyramids Entrance.",$u['name']);
				$time = date("Y-m-d H:i:s");
			}elseif ($fightnum==2){
				$u['hitpoints'] = 1;
				output("You can feel the guardian throw you bodily back from the Pyramid");
				$exploss = round($u['experience']*.05);
				$u['experience']-=$exploss;
				output("You have lost %s experience",$exploss);
				output("You start to come too a bit more, until your body hits the hard surface of the village square");
				addnews("%s `Gwas thrown bodily from a Pyramid.",$u['name']);
			}elseif ($fightnum==3){
				$u['hitpoints'] = 1;
				output("You can feel the God Nehebkau throw you bodily out of the Pyramid");
				$exploss = round($u['experience']*.05);
				$u['experience']-=$exploss;
				output("You have lost %s experience",$exploss);
				output("You start to come too a bit more, until your body hits the hard surface of the village square");
				addnews("%s `Gwas hurled from a Pyramid by a God.",$u['name']);
			}
			$u['hitpoints']=1;
			$time = date("Y-m-d H:i:s");
