	...
	if ($dam>0){
		...
		if ($ophp<=0){
			...
			if ($optime<>905){
				...
				output("and you kill them, you earn a clanpoint for your clan");
		...
		if ($hp<=0){
			...
			if ($attime<>905){
				...
				output(" and have been killed, you have just lost a clan point for your clan");
				output_notl("`n");
				$cw=get_module_objpref("clans",$clanid,"clanwins","clanpyramid")-1;
				if ($cw<=0){
					$cw=0;
...
function warriors_list($p){
   ...
   rawoutput("</table>");
   addnav("Return to Pyramid", "runmodule.php?module=clanpyramid&op=move&move=return&wall=$square&p=$p");
}
?>