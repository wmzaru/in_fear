...
if($move=="thronec"){
	...
	if ($clanid<>$owned1){
		output("You have captured the Pyramid in the name of your clan");
		output_notl("`n");
		...
		output_notl("`n`n");
		output("You receive %s Clan Points",$mpoints);
		...
		//}
		output_notl("`n`n`^");
		output("Your clans walls are $wallhp");
	...
	output("Your clan is successful and the Pyramid has been captured.");
	output_notl("`n");
}
villagenav();
page_footer();
...
if ((($squarenew+1)==$doort1 || ($squarenew+13)==$doort1 || ($squarenew-1)==$doort1 || ($squarenew-13)==$doort1) && $defender<>1){
	if (get_module_objpref("clans",$clanid,"doort","clanpyramid")>0){
		...
		output("Your clan has already taken down the throne.");
		output_notl("`n");
