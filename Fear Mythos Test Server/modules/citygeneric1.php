<?php
function citygeneric1_getmoduleinfo(){
	$info = array(
		"name"=>"City - Generic 1",
		"version"=>"1.01",
		...
		"download"=>"",
		"settings"=>array(
			"Generic City 1 Settings,title",
			"villagename"=>"Name for the village|Corinvale",
		...
		"prefs"=>array(
			"Generic City 1 Preferences,title",
...
function citygeneric1_uninstall(){
	global $session;
...
function citygeneric1_dohook($hookname,$args){
	...
	case "pointsdesc":
		...
		break;
	case "lodge":
		if (get_module_setting("points")==2 && get_module_pref("donated")==0) addnav(array("Unlimited Access to %s (%s Points)",get_module_setting("villagename"),get_module_setting("cost")),"runmodule.php?module=citygeneric1&op=lodge");
		elseif (get_module_setting("points")==3 && get_module_pref("donated")==0) addnav(array("Access %s for %s Days (%s Points)",get_module_setting("villagename"),get_module_setting("xdays"),get_module_setting("cost")),"runmodule.php?module=citygeneric1&op=lodge");
		elseif (get_module_setting("points")==4 && get_module_pref("donated")==0) addnav(array("Access %s this DK (%s Points)",get_module_setting("villagename"),get_module_setting("cost")),"runmodule.php?module=citygeneric1&op=lodge");
	break;
	case "pvpwin":
		if ($session['user']['location'] == $city) {
			$args['handled']=true;
			addnews("`4%s`3 defeated `4%s`3 in fair combat in a dark and filthy alleyway within %s.", $session['user']['name'],$args['badguy']['creaturename'], $args['badguy']['location']);
		}
	break;
	case "pvploss":
		if ($session['user']['location'] == $city) {
			$args['handled']=true;
			addnews("`%%s`5 has been slain while attacking `^%s`5 in a dark and filthy alleyway within `&%s`5.`n%s`0", $session['user']['name'], $args['badguy']['creaturename'], $args['badguy']['location'], $args['taunt']);
		}
		break;
	case "pvpstart":
		if ($session['user']['location'] == $city) {
			$args['atkmsg'] = "`4You wander through the city's network of alleys, to where a large group of wanderers are getting their worth of whores or gigolos. Just out of the cities light, in the darkness and away from the others, some foolish warriors have bedded down for the night...`n`nYou have `^%s`4 PvP fights left for today.`n`n";
			$args['schemas']['atkmsg'] = 'module-citygeneric1';
		}
		break;
	case "travel":
	...
	case "changesetting":
		...
		if ($args['setting']=="villagename" && $args['module']=="citygeneric1") {
		...
		break;
	case "validlocation":
			$args[$city]="village-citygeneric1";
		break;
	case "moderate":
			tlschema("commentary");
			$args["village-citygeneric1"]=sprintf_translate("%s Village", $city);
			tlschema();
		break;
	case "villagetext":
		if ($session['user']['location'] == $city){
			...
			if ((get_module_setting("points")==1 && $cost > $session['user']['donation']) || (get_module_setting("points")==2 && get_module_pref("donated")==0))  $args['text']="`\$`c`@`bThe redlight city named $city`b`@`c`n`n`2 The residents of $city busy themselves with various tasks of everyday, or everynight, life.  `n`n`^It's a rare opportunity for you to visit $city today.  You can access $city everyday if you would stop at the Lodge to donate to the site!`0";
			elseif (get_module_setting("points")==3 && get_module_pref("donated")==0) $args['text']="`\$`c`@`bThe redlight city named $city`b`@`c`n`n`2 The residents of $city busy themselves with various tasks of everyday, or everynight, life.  `n`n`^It's a rare opportunity for you to visit $city today.  You can access $city everyday for $xdays days if you would stop at the Lodge to donate to the site!`0";
			elseif (get_module_setting("points")==4 && get_module_pref("donated")==0) $args['text']="`\$`c`@`bThe redlight city named $city`b`@`c`n`n`2 The residents of $city busy themselves with various tasks of everyday, or everynight, life.  `n`n`^It's a rare opportunity for you to visit $city today.  You can access $city everyday for until you kill the dragon if you would stop at the Lodge to donate to the site!`0";
			else $args['text']="`\$`c`@`bThe redlight city named $city`b`@`c`n`n`2 The residents of $city busy themselves with various tasks of everyday, or everynight, life.  Small street urchins scurry underfoot as you make your way through the maze of streets and alleys.`n`nMost of the population of $city seem to stare at you and you feel self conscience of your looks as members of the opposite sex wink at you.  Unsure of yourself, you only just notice that the whole city is a redlight district.  You look around for something to do..`n`nYou get the feeling as if you can let your self go slightly wild.`n`n";
            $args['schemas']['text'] = "module-citygeneric1";
			$args['clock']="`n`7In the distance, a clocktower chimes `&%s`7.`n";
            $args['schemas']['clock'] = "module-citygeneric1";
			if (is_module_active("calendar")) {
				$args['calendar'] = "`n`2Overheard whispers suggest that it is `&%s`2, `&%s %s %s`2.`n";
				$args['schemas']['calendar'] = "module-citygeneric1";
			}
			$args['title']=array("%s Village", $city);
			$args['schemas']['title'] = "module-citygeneric1";
			$args['sayline']="brags";
			$args['schemas']['sayline'] = "module-citygeneric1";
			$args['talk']="`n`&Nearby some visitors brag:`n";
			$args['schemas']['talk'] = "module-citygeneric1";
			$args['newest'] = "";
			...
			$args['schemas']['newest'] = "module-citygeneric1";
			$args['gatenav']="Village Gates";
			$args['schemas']['gatenav'] = "module-citygeneric1";
			$args['fightnav']="Nearby Forest";
			$args['schemas']['fightnav'] = "module-citygeneric1";
			$args['marketnav']="Redlight District";
			$args['schemas']['marketnav'] = "module-citygeneric1";
			$args['tavernnav']="Shadey Alley";
			$args['schemas']['tavernnav'] = "module-citygeneric1";
			$args['section']="village-citygeneric1";
			$args['infonav']="Village Council";
			$args['schemas']['infonav'] = "module-citygeneric1";
		}
		break;
	...
	case "village":
		...
		if ($session['user']['location']!= $city && get_module_setting("announce")==1 && $session['user']['dragonkills']>=get_module_setting("mindk")){
			output("`n`^`c`bAnnouncement:  The city of %s is open today for %s citizens to visit.",get_module_setting("villagename"),$text);
...
if ($op=="lodge"){
	...
	if ($op2==""){
		...
		if (get_module_setting("mindk")>$session['user']['dragonkills']){
			...
			addnav(array("Purchase Pass to %s (%s Points)",$place,$cost),"runmodule.php?module=citygeneric1&op=lodge&op2=confirm");
		}
	}elseif ($op2=="confirm"){
