<?php
function citygeneric2_getmoduleinfo(){
	$info = array(
		"name"=>"City - Generic 2",
		"version"=>"1.01",
		...
		"download"=>"",
		"settings"=>array(
			"Generic City 2 Settings,title",
			"villagename"=>"Name for the village|Geneva",
		...
		"prefs"=>array(
			"Generic City 2 Preferences,title",
...
function citygeneric2_uninstall(){
	global $session;
...
function citygeneric2_dohook($hookname,$args){
	...
	case "pointsdesc":
		...
		break;
	case "lodge":
		if (get_module_setting("points")==2 && get_module_pref("donated")==0) addnav(array("Unlimited Access to %s (%s Points)",get_module_setting("villagename"),get_module_setting("cost")),"runmodule.php?module=citygeneric2&op=lodge");
		elseif (get_module_setting("points")==3 && get_module_pref("donated")==0) addnav(array("Access %s for %s Days (%s Points)",get_module_setting("villagename"),get_module_setting("xdays"),get_module_setting("cost")),"runmodule.php?module=citygeneric2&op=lodge");
		elseif (get_module_setting("points")==4 && get_module_pref("donated")==0) addnav(array("Access %s this DK (%s Points)",get_module_setting("villagename"),get_module_setting("cost")),"runmodule.php?module=citygeneric2&op=lodge");
	break;
	case "pvpwin":
	...
	case "pvpstart":
		if ($session['user']['location'] == $city) {
			...
			$args['schemas']['atkmsg'] = 'module-citygeneric2';
		}
		break;
	case "travel":
	...
	case "changesetting":
		...
		if ($args['setting']=="villagename" && $args['module']=="citygeneric2") {
		...
		break;
	case "validlocation":
			$args[$city]="village-citygeneric2";
		break;
	case "moderate":
			tlschema("commentary");
			$args["village-citygeneric2"]=sprintf_translate("%s Village", $city);
			tlschema();
		break;
	case "villagetext":
		if ($session['user']['location'] == $city){
            ...
            $args['schemas']['text'] = "module-citygeneric2";
			$args['clock']="`n`7A small sundial at the center of the village reads `&%s`7.`n";
            $args['schemas']['clock'] = "module-citygeneric2";
			if (is_module_active("calendar")) {
				$args['calendar'] = "`n`2Overheard whispers suggest that it is `&%s`2, `&%s %s %s`2.`n";
				$args['schemas']['calendar'] = "module-citygeneric2";
			}
			$args['title']=array("%s Village", $city);
			$args['schemas']['title'] = "module-citygeneric2";
			$args['sayline']="brags";
			$args['schemas']['sayline'] = "module-citygeneric2";
			$args['talk']="`n`&Nearby some visitors brag:`n";
			$args['schemas']['talk'] = "module-citygeneric2";
			$args['newest'] = "";
			...
			$args['schemas']['newest'] = "module-citygeneric2";
			$args['gatenav']="Village Gates";
			$args['schemas']['gatenav'] = "module-citygeneric2";
			$args['fightnav']="Nearby Forest";
			$args['schemas']['fightnav'] = "module-citygeneric2";
			$args['marketnav']="Village Market";
			$args['schemas']['marketnav'] = "module-citygeneric2";
			$args['tavernnav']="Drunkard's Lane";
			$args['schemas']['tavernnav'] = "module-citygeneric2";
			$args['section']="village-citygeneric2";
			$args['infonav']="Village Council";
			$args['schemas']['infonav'] = "module-citygeneric2";
		}
		break;
...
if ($op=="lodge"){
	...
	if ($op2==""){
		...
		if (get_module_setting("mindk")>$session['user']['dragonkills']){
			...
			addnav(array("Purchase Pass to %s (%s Points)",$place,$cost),"runmodule.php?module=citygeneric2&op=lodge&op2=confirm");
		}
	}elseif ($op2=="confirm"){
