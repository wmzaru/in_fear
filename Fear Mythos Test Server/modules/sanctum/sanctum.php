...
if ($op=="enter") {
	if (get_module_pref("member")==-1){
		...
		output("`n`n `&You reach to show her the `&I`)nvitation `&S`)croll`& you received in `5Silver Twilight Lodge`& but she shakes her head softly.");
		...
		output("You will learn about those in good time.'`n`n'This offer will not come often.");
		...
		addnav("Return");
		addnav("`&Return to Arkham","village.php");
	}else{
...
if ($op=="tattoo") {
	...
	modulehook ("sanctum-tattoo"); 
	addnav("`&T`)he `&P`)arlor","runmodule.php?module=sanctum&op=enter");
...
if ($op=="memberlist") {
	...
	addnav("The Order");
	addnav("`&The Lounge","runmodule.php?module=sanctum&op=enter");
	addnav("Arkham");
	addnav("Return to Arkham","village.php");
}
if ($op=="decline") {
