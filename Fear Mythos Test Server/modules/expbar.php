...
function expbar_getmoduleinfo(){
	$info = array(
		...
		"prefs"=>array(
			"Experience Bar User Settings, title",
			"user_icon"=>"Use Icon When Ready to Challenge Master?,bool|0",
		)
	);
	return $info;
}
...
function expbar_dohook($hookname,$args){
	...
	case "charstats":
		...
		if ($exp >= $req) {
			if (get_module_pref("user_icon") == 0) {
				$bar = "<table style='border: solid 1px #000000;' bgcolor='red'  cellpadding='0' cellspacing='0' width='70' height='5'><tr><td width='$pct%' bgcolor='blue'></td><td width='$nonpct%'></td></tr></table>";
			}else{
			$bar = "<img src=modules/expbar/thumb.gif>";
			}
		} else {
			$bar = "<table style='border: solid 1px #000000;' bgcolor='red'  cellpadding='0' cellspacing='0' width='70' height='5'><tr><td width='$pct%' bgcolor='white'></td><td width='$nonpct%'></td></tr></table>";
		}
