...
function Evilriddles_getmoduleinfo(){
	$info = array(
		"name"=>"Riddling Gnome",
		"version"=>"1.2",
		"author"=>"Joe Naylor Edited by Zachary Bridges",
...
function Evilriddles_install(){
	if (db_table_exists(db_prefix("Evilriddles"))) {
		debug("Evilriddles table already exists");
	} else {
		debug("Creating Evilriddles table.");
		// This is pulled out to another file just because it's so big.
		// no reason to parse it every time this module runs.
		require_once("modules/Evilriddles/riddles_install.php");
	}
...
function Evilriddles_uninstall(){
	debug("Dropping Evilriddles table");
	$sql = "DROP TABLE IF EXISTS " . db_prefix("Evilriddles");
	db_query($sql);
	return true;
}

function Evilriddles_dohook($hookname,$args){
	global $session;
	switch($hookname) {
	case "superuser":
		if (($session['user']['superuser'] & SU_EDIT_RIDDLES) ||
			...
			addnav("Riddle Editor",
					"runmodule.php?module=Evilriddles&act=editor&admin=true");
		}
		break;
	}
	return $args;
...
function Evilriddles_runevent($type)
{
	require_once("lib/increment_specialty.php");
	global $session;
	// We assume this event only shows up in the forest currently.
	$from = "forest.php?";
	$session['user']['specialinc'] = "module:Evilriddles";
	...
	if ($op=="" || $op=="search"){
		...
		output("`6\"`@I'll give you a 10 gems,`6\" he says, \"`@if you can think and answer my riddle!`6\"`n`n");
		output("`6He loses himself momentarily in a fit of giggling, then contains his excitement for a moment and continues.`n`n");
		output("`6\"`@But if ere long your guess is wrong, then you will die`6\"`n");
		output("`6`nDo you accept his challenge?`n`n");
		addnav(" Evil Riddle Gnome");
		...
		if ($subop!="answer"){
			...
			if (!strpos($rid, "Riddle")) {
				$sq1 = "SELECT * FROM " . db_prefix("Evilriddles") . " ORDER BY rand(".e_rand().")";
			}else{
				// 6 letters in "Riddle"
				$rid = substr($rid, -1*(strlen($rid)-6));
				$sq1 = "SELECT * FROM " . db_prefix("Evilriddles") . " WHERE id=$rid";
			}
			$result = db_query($sq1);
			...
			addnav("Return to the Shades","shades.php");
				...
				output("Suddenly he takes a knife and stabs you in the back. You cough up a bit of blood as you stare into his souless eyes. Gazing upon his evil smile.");
...
function Evilriddles_run(){
	$act = httpget("act");
	if ($act=="editor") riddles_editor();
}

function Evilriddles_editor() {
	global $session;
	...
	superusernav();
	addnav("Riddle Editor");
	addnav("Riddle Editor Home","runmodule.php?module=Evilriddles&act=editor&admin=true");
	addnav("Add a riddle","runmodule.php?module=Evilriddles&act=editor&op=edit&admin=true");
	if ($op=="save"){
		$id = httppost('id');
		$riddle = trim(httppost('riddle'));
		$answer = trim(httppost('answer'));
		if ($id > "") {
			$sql = "UPDATE " . db_prefix("Evilriddles") . " SET riddle='".nltoappon($riddle)."', answer='$answer' WHERE id='$id'";
		}else{
			$sql = "INSERT INTO " . db_prefix("Evilriddles") . " (riddle,answer,author) VALUES('".nltoappon($riddle)."','$answer','{$session['user']['login']}')";
		}
		db_query($sql);
		...
		$sql = "DELETE FROM " . db_prefix("Evilriddles") . " WHERE id='$id'";
		db_query($sql);
		$op = "";
		httpset("op", "");
	...
	if ($op==""){
		$sql = "SELECT * FROM " . db_prefix("Evilriddles");
		$result = db_query($sql);
		...
		for ($i=0;$i<db_num_rows($result);$i++){
			...
			$elink = "runmodule.php?module=Evilriddles&act=editor&op=edit&id=".$row['id']."&admin=true";
			$dlink = "runmodule.php?module=Evilriddles&act=editor&op=del&id=".$row['id']."&admin=true";
			output_notl("[");
		...
		$sql = "SELECT * FROM " . db_prefix("Evilriddles") . " WHERE id='$id'";
		$result = db_query($sql);
		rawoutput("<form action='runmodule.php?module=Evilriddles&act=editor&op=save&admin=true' method='POST'>",true);
		addnav("","runmodule.php?module=Evilriddles&act=editor&op=save&admin=true");
		...
		output("`\$NOTE:  Evilriddles are displayed in the language they are stored in the database.");
