...
function dag_run_private(){
	...
	if ($op=="list"){
		...
		$sql = "SELECT bountyid,amount,target,setter,setdate,clan FROM " . db_prefix("bounty") . " WHERE status=0 AND setdate<='".date("Y-m-d H:i:s")."' ORDER BY bountyid ASC";
		$result = db_query($sql);
		...
		$clan = translate_inline("Clan name");
		rawoutput("<tr class='trhead'><td><b>$amount</b></td><td><b>$level</b></td><td><b>$name</b></td><td><b>$loc</b></td><td><b>$sex</b></td><td><b>$alive</b></td><td><b>$last</b></td><td><b>$clan</b></td>");
		$listing = array();
		$totlist = 0;
		for($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
			if (get_module_setting("update")) {
				dag_check_clan($row['target'],$row['clan']);
				$sql = "SELECT bountyid,amount,target,setter,setdate,clan FROM " . db_prefix("bounty") . " WHERE status=0 AND setdate<='".date("Y-m-d H:i:s")."' ORDER BY bountyid ASC";
				$result = db_query($sql);
				$row = db_fetch_assoc($result);
			}
			$amount = (int)$row['amount'];
			$sql = "SELECT name,alive,sex,level,laston,loggedin,lastip,location,clanid,clanrank FROM " . db_prefix("accounts") . " WHERE acctid={$row['target']}";
			...
			if ($yesno==0) {
				...
				$listing[] = array('Amount'=>$amount,'Level'=>$row2['level'],'Name'=>$row2['name'],'Location'=>$row2['location'],'Sex'=>$row2['sex'],'Alive'=>$row2['alive'],'LastOn'=>$row2['laston'], 'LoggedIn'=>$loggedin, 'Clanid'=>$row2['clanid'], 'Clanname'=>'');
				$totlist = $totlist + 1;
			}
		}
		$sort = httpget("sort");
		...
		elseif ($sort != "")
			...
			usort($listing, 'dag_sortbountieslevel');
		for($i=0;$i<$totlist;$i++) {
			if ($listing[$i]['Clanid'] == '0' || $row2['clanrank'] == '0') { $listing[$i]['Clanname'] = "None"; }
			else {
				$sql3 = "SELECT clanname FROM `".db_prefix("clans")."` WHERE `clanid` = ".$listing[$i]['Clanid']." LIMIT 1";
				$result3 = db_query($sql3);
				$row3 = db_fetch_assoc($result3);
				$listing[$i]['Clanname'] = $row3['clanname'];
			}
			...
			rawoutput("</td><td>");
			output_notl("`^%s`0", $listing[$i]['Clanname']);
			rawoutput("</td></tr>");
		}
		rawoutput("</table>");
		// ***END ADDING***
		...
		if (httpget('subfinal')==1){
			$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience,clanid,clankrank FROM " . db_prefix("accounts") . " WHERE name='".addslashes(rawurldecode(stripslashes(httppost('contractname'))))."' AND locked=0";
		}else{
			...
			$sql = "SELECT acctid,name,login,level,locked,age,dragonkills,pk,experience,clanid FROM " . db_prefix("accounts") . " WHERE name LIKE '".addslashes($name)."' AND locked=0";
		}
		$result = db_query($sql);
		if (db_num_rows($result) == 0) {
			...
			if ($row['locked']) {
			...
			} elseif ($row['clanid'] == $session['user']['clanid'] && $row['clanid'] != "" && $row2['clanrank'] != '0' && $session['user']['clanrank'] != '0') {
				output("Dag Durnick stares at you angrily, `7\"Ye be wanting to take out a contract on yerfriend? I aught to be puttin' a contract on YE instead!");
			} else {
				// All good!
				...
				if ($amt < $min) {
					...
					$sql = "INSERT INTO ". db_prefix("bounty") . " (amount, target, setter, setdate, clan) VALUES ($amt, ".$row['acctid'].", ".(int)$session['user']['acctid'].", '".date("Y-m-d H:i:s",$setdate)."', ".$row['clanid'].")";
					db_query($sql);
					// ***END ADD***
