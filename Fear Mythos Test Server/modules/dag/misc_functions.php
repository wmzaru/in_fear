...
function dag_manage(){
	...
	addnav("U?Update Clans","runmodule.php?module=dag&manage=true&admin=true&op=clan");
	...
	if ($op == "") {
		...
		$sql = "SELECT * FROM " . db_prefix("bounty") . " WHERE status=0 ORDER BY bountyid ASC";
		$result = db_query($sql);
		...
		$clan = translate_inline("Clanname");
		rawoutput("<tr class='trhead'><td><b>$amt</b></td><td><b>$lev</b></td><td><b>$name</b></td><td><b>$loc</b></td><td><b>$sex</b></td><td><b>$alive</b></td><td><b>$last</b></td><td><b>$clan</b></td>");
		$listing = array();
		$totlist = 0;
		for($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
				$amount = (int)$row['amount'];
				$sql = "SELECT name,alive,sex,level,laston,loggedin,lastip,uniqueid,clanid FROM " . db_prefix("accounts") . " WHERE acctid={$row['target']}";
				...
				if ($yesno==0) {
					$listing[] = array('Amount'=>$amount,'Level'=>$row2['level'],'Name'=>$row2['name'],'Location'=>$row2['location'],'Sex'=>$row2['sex'],'Alive'=>$row2['alive'],'LastOn'=>$row2['laston'],'Clan'=>$row2['clanid'],'clan'=>'');
					$totlist = $totlist + 1;
				}
				$sql3 = "SELECT clanname FROM `".db_prefix("clans")."` WHERE `clanid` = ".$listing[$i]['Clan']." LIMIT 1";
				$result3 = db_query($sql3);
				$row3 = db_fetch_assoc($result3);
				$listing[$i]['clan'] = $row3['clanname'];
		}
		...
		for($i=0;$i<$totlist;$i++) {
			...
			rawoutput("</td><td>");
			output_notl("`^%s`0", $listing[$i]['clan']);
			rawoutput("</td></tr>");
		}
		rawoutput("</table>"); 
		...
		if (db_num_rows($result) == 0) {
			...
			if ($amt <= 0) {
				...
				// All good!
				$sql = "INSERT INTO " . db_prefix("bounty") . " (amount, target, setter, setdate, clan) VALUES ($amt, ".$row['acctid'].", 0, '".date("Y-m-d H:i:s")."', ".$row['clanid'].")";
				db_query($sql);
				output("Bounty added!");
			}
		}
		...
		$sql = "SELECT bountyid,amount,target,setter,setdate,status,winner,windate,clan FROM " . db_prefix("bounty").$t.$s.$d;
		$result = db_query($sql);
		rawoutput("<table border=0 cellpadding=2 cellspacing=1 bgcolor='#999999'>");
		$id = translate_inline("ID");
		...
		$clan = translate_inline("Clan id");

		rawoutput("<tr class='trhead'><td><b>$id</b></td><td><b>$amt</b></td><td><b>$targ</b></td><td><b>$set</b></td><td><b>$sdate</b></td><td><b>$stat</b></td><td><b>$win</b></td><td><b>$wdate</b></td><td><b>$clan</b></td><td>$ops</td></tr>");
		for($i=0;$i<db_num_rows($result);$i++){
			$row = db_fetch_assoc($result);
			if ($row['target']==0) {
...
function dag_check_clan($id,$clan) {
	$sql = "SELECT `clan` FROM " . db_prefix("bounty") . " WHERE `target`=".$id.";";
	$result = db_query($sql);
	if(db_num_rows($result) == 0) return;
	$row = db_fetch_assoc($result);
	if($row['clan']==$clan) return;
	$sql = "UPDATE " . db_prefix("bounty") . " SET clan='$clan' WHERE target='$id';";
	db_query($sql);
	return;
}
?>