<?php
			if (get_module_setting("dktax")==1) {
				...
				if (get_module_setting("allowgoldinvault")==1) {
					...
					$msg = sprintf("::`7killed `@the Green dragon`7 and the clan has been taxed `$%s `^gold.", ($gold-$goldaftertax));
				}
				if (get_module_setting("allowgemsinvault")==1) {
					...
					$msg = sprintf("::`7killed `@the Green dragon`7 and the clan has been taxed `$%s `@gems.", ($gems-$gemsaftertax));
				}	
				if (get_module_setting("allowgemsinvault")==1 and get_module_setting("allowgoldinvault")==1) {
					$msg = sprintf("::`7killed `@the Green dragon`7 and the clan has been taxed `$%s `^gold`7 and `$%s `@gems.", ($gold-$goldaftertax), ($gems-$gemsaftertax));
				}
				$name = $session['user']['name'];
				if ($gold>0) {
					if (get_module_setting("taxannouncement")==1) injectcommentary("clan-".$session['user']['clanid'], "", $msg, $schema=false);
					output("`nYour clan has to pay taxes because of the dragon kill.");
					...
					while ($row = db_fetch_assoc($result)) {
						if (get_module_pref('showNot','clanvault',$row['acctid'])==1) {
							$subject = array("Dragon Kill Tax");
							$msg = array("A member of your clan, %s, `^has slain the dragon which means that your clan has to pay a special dragon kill tax.`nConsidering the amount of members in your clan the tax is %s gold and %s gems.`n`nSincerely,`nTaxman", $session['user']['name'], ($gold-$goldaftertax), ($gems-$gemsaftertax));
