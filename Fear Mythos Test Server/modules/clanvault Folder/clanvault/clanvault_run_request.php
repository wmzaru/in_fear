			...
			switch ($action) {
				case "form":
					if (get_module_pref("requests") > 0)
					{
						...
						else
						{
							...
							output("Try again after the clan leaders have made their decision about your current request in a line.");
						}
					}
					else
					{
				...
				case "request":
					...
					if ($hit==0) {
						...
						output("Try again later. Maybe the clan leaders have shortened the line then.");
...
//END
				...
				case "accept":
					...
					if ((($gold >= $amount) and $type == "gold") or (($gems >= $amount) and $type == "gems"))
					{
						...
						$msg = array("Your request for %s %s has been accepted by the clan management.`n`n%s of the clan`n%s", $amount, $type, $ranks[$session['user']['clanrank']], $session['user']['name']);
						...
						while ($row = db_fetch_assoc($result))
						{
							if (get_module_pref('showNot','clanvault',$row['acctid'])==1)
							{
								...
								$msg = array("%s requested for %s %s and I have accepted it.`n`n%s of the clan`n%s", $name, $amount, $type, $ranks[$session['user']['clanrank']], $session['user']['name']);
				...
				case "deny":
					...
					$id = httpget("id");
					$subject = translate_inline("About your request");
					$msg = array("Your request for %s %s has been denied by the clan management.`nReason for this is probably too high amount or too many request from same person and too often.`n`n%s of the clan`n%s", $amount, $type, $ranks[$session['user']['clanrank']], $session['user']['name']);
					systemmail($id, $subject, $msg);
					set_module_pref("hasrequested",0,"clanvault",$id);
...
//END RPGee.com
					...
					while ($row = db_fetch_assoc($result)) {
						if (get_module_pref('showNot','clanvault',$row['acctid'])==1) {
							...
							$msg = array("%s requested for %s %s and I have denied it.`n`n%s of the clan`n%s", $name, $amount, $type, $ranks[$session['user']['clanrank']], $session['user']['name']);
							systemmail($row['acctid'], $subject, $msg);
						}
					}
