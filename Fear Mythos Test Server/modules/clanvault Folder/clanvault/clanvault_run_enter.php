...
if (get_module_setting("allowgemsinvault")==0 and get_module_setting("allowgoldinvault")==0) {
   output("The vault exists but the gods won't allow the clan members to use it!");
} else {
	if ($session['user']['clanrank'] >= CLAN_OFFICER) {
		output("Guards of the Vault let you in the vault. This is the place where all gold and gems that members have donated exists.`n");
...
output("`nThe guards are allowed to let only leaders of the clan in that have been a member of the clan for 24 real-time hours.`n");
