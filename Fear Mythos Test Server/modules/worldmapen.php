...
function worldmapen_editor(){
	...
	if ($act == "save"){
        ...
        addnav("E?Return to World Map Editor","runmodule.php?module=worldmapen&op=edit&admin=true");
            ...
            // -----------------------------------------------------------------------
            output("<script language=\"javascript\">",true);
            output("colors = [\"#008800\", \"#005500\", \"#0000CC\", \"#000066\", \"#DDDD33\", \"#669900\", \"#999999\", \"#FFFFFF\"];",true);
            output("cRGB = [];",true);
            output("function toRGB(color){",true);
            output("var rgb = \"rgb(\" + parseInt(color.substring(1,3), 16) + \", \" + parseInt(color.substring(3,5), 16) + \", \" + parseInt(color.substring(5,8), 16) + \")\";   ",true);
            output("return rgb;",true);
            output("}",true);
            output("for(var i=0; i<colors.length; i++){",true);
            output("cRGB[i] = toRGB(colors[i]);",true);
            output("}",true);
            output("function changeColor(target){",true);
            output("var swapper = navigator.appVersion.indexOf(\"MSIE\")!=-1 ? toRGB(document.getElementById(target).style.backgroundColor) : document.getElementById(target).style.backgroundColor;",true);
            output("var set = false;",true);
            output("var xx;",true);
            output("for(var i=0; i<cRGB.length; i++){",true);
            output("if(swapper == cRGB[i]){",true);
            output("if(((i+1)) >= cRGB.length){",true);
            output("xx = 0;",true);
            output("}else{",true);
            output("xx = i+1;",true);
            output("}",true);
            output("document.getElementById(target).style.background = colors[xx];",true);
            output("document.getElementById(target+\"b\").value = xx;",true);
            output("set = true;",true);
            output("i=cRGB.length;",true);
            output("}",true);
            output("}",true);
            output("set ? null : (document.getElementById(target).style.background = colors[1], document.getElementById(target+\"b\").value = 1);",true);
            output("}",true);
            output("</script>",true);
            // -----------------------------------------------------------------------
...
function worldmapen_viewmap($showloc){
    ...
    if (get_module_setting("showcompass") == 1){
        ...
        for ($y = $sizeY;$y > 0;$y--){
          ...
          rawoutput("<td width='20' align='right'>$y&nbsp;&nbsp;</td>");
                for ($x = 1;$x <= $sizeX;$x++){
                    ...
                    output("<td id=\"".$y."".$x."".$y."\" onclick=\"changeColor(this.id);\" bgcolor=".$cellcolor." align=center valign=middle height=25 width=30><input type=\"hidden\" id=\"".$y."".$x."".$y."b\" name=\"".$x.".".$y."\" value=\"$cellvalue\"/>",true);
                    foreach($vloc as $loc=>$val) {
                  if ($y == get_module_setting($loc."Y") && $x == get_module_setting($loc."X")) {
                      output_notl("%s", substr($loc, 0, 3));
                      $city = true;
              ...
              if (!$city) {
                ...
                output("<td colspan='2'></td>",true);
                for ($x = 1;$x <= get_module_setting("worldmapsizeX");$x++){
                    output("<td align=middle>$x</td>",true);
                }
            output("</tr><tr><td colspan=2></td><td align=center colspan=".get_module_setting("worldmapsizeX").">`6`bX`b</td></tr>",true);
            output("</table>`c",true);
            output("<center><input type=submit value=\"Save Terrain\">",true);
            output("</form></center>",true);
        ...
        worldmapen_viewmapkey(true,true);
            ...
            output("<td bgcolor=".get_module_setting("colorPlains")."><img src='images/trans.gif' height=5 width=10></td><td>Plains - ".get_module_setting("moveCostPlains")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorForest")."><img src='images/trans.gif' height=5 width=10></td><td>Dense Forest - ".get_module_setting("moveCostForest")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorRiver")."><img src='images/trans.gif' height=5 width=10></td><td>River - ".get_module_setting("moveCostRiver")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorOcean")."><img src='images/trans.gif' height=5 width=10></td><td>Ocean - ".get_module_setting("moveCostOcean")."</td>",true);
            rawoutput("</tr><tr>");
            output("<td bgcolor=".get_module_setting("colorDesert")."><img src='images/trans.gif' height=5 width=10></td><td>Desert - ".get_module_setting("moveCostDesert")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorMount")."><img src='images/trans.gif' height=5 width=10></td><td>Mountains - ".get_module_setting("moveCostMount")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorSwamp")."><img src='images/trans.gif' height=5 width=10></td><td>Swamp - ".get_module_setting("moveCostSwamp")."</td>",true);
			output("<td bgcolor=".get_module_setting("colorSnow")."><img src='images/trans.gif' height=5 width=10></td><td>Snow - ".get_module_setting("moveCostSnow")."</td>",true);
...
function worldmapen_viewmapkey($showloc,$small){
    ...
    rawoutput("<tr><td height=1 bgcolor=000000></td><tr>");
    rawoutput("<tr>");
    output("<td>`b`6$mapkey`0`b</td>",true);
    rawoutput("</tr>");
    rawoutput("</table>");
    $loc = get_module_pref("worldXYZ");
    list($worldmapX, $worldmapY, $worldmapZ) = explode(",", $loc);
	...
	if (get_module_setting("showcities") == 1){
        ...
        foreach($vloc as $loc=>$val) {
            rawoutput("<tr><td>");
            output("%s</td>",substr($loc,0,3),true);
            output("<td>= $loc</td></tr>",true);
        }
        rawoutput("</table>");
	}	
    if($small){
        ...
        if (get_module_setting("enableTerrains") ==1){
			...
			output("<img src='images/trans.gif' height=5 width=10></td><td>`0Current Terrain:  `b`^$currentTerrain `b`^`n<hr></td>",true);
            rawoutput("<tr>");
            output("<td>`&`b$terrains`b`&</td>",true);
            rawoutput("</tr><tr>");
            output("<td>(Terrain Color, Terrain Type, Terrain Movement Cost)</td>",true);
            rawoutput("</tr><tr><br>");
            rawoutput("<table cellpadding=2 cellspacing=5 border=0><tr>");
            output("<td bgcolor=".get_module_setting("colorPlains")."><img src='images/trans.gif' height=5 width=10></td><td>Plains - ".get_module_setting("moveCostPlains")."</td>",true);
            output("<td bgcolor=".get_module_setting("colorForest")."><img src='images/trans.gif' height=5 width=10></td><td>Dense Forest - ".get_module_setting("moveCostForest")."</td>",true);
