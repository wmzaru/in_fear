...
function sanctum_getmoduleinfo(){
	$info = array(
		...
		"settings"=>array(
			...
			"chance"=>"Chance to encounter sheriff in the forest:,range,1,100,1|30",
		),
		"prefs"=>array(
			"Order of the Inner Sanctum,title",
...
function sanctum_chance() {
	$ret= get_module_setting('chance','sanctum');
	if (get_module_pref("encounter")==1) $ret=0;
	return $ret;
}
...
function sanctum_dohook($hookname,$args){
	...
	switch($hookname){
		...
		break;
		case "village":
			if ($session['user']['location'] == get_module_setting("sanctumloc")){
				tlschema($args['schemas']['tavernnav']);
				addnav($args['tavernnav']);
				tlschema();
				addnav("Order of the Inner Sanctum","runmodule.php?module=sanctum&op=enter");
			}
		break;
		case "bioinfo":
...
function sanctum_runevent($type){
	...
	if ($op=="yes"){
		...
		output("`n`nHe hands you a scroll and points you to the headquarters in %s.",get_module_setting("sanctumloc"));
