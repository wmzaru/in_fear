...
function badnaved_players_dohook($hookname,$args)
{
	...
	if( $session['user']['superuser'] & SU_FIX_BADNAVS )
	{
		...
		$count = ( $count == 0 ) ? 0 : '<span style="color:red;font-weight:bold;text-decoration:blink;">'.$count.'</span>';
		addnav('Actions');
...
function badnaved_players_run()
{
	...
	if( $op == 'fix' )
	{
		...
		if( is_array($playerids) && !empty($playerids) )
		{
			...
			{
				db_query("UPDATE " . db_prefix('accounts') . " SET allowednavs='', restorepage='', specialinc='' WHERE acctid='$acctid'");
				db_query("DELETE FROM " . db_prefix('accounts_output') . " WHERE acctid='$acctid'");
				$i++;
			}
