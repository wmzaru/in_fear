...
"vessaloc"=>"Where does Vessa appear,location|".getsetting("villagename", LOCATION_FIELDS)



),
"prefs"=>array(
"Vessa User Preferences,title",
"gemsbuytoday"=>"How many gems bought today?,int|0",
"gemsselltoday"=>"How many gems sold today?,int|0",
),
);
return $info;
}
...
case "village":
	if ($session['user']['location'] == get_module_setting("vessaloc")) {
		tlschema($args['schemas']['marketnav']);
		addnav($args['marketnav']);
		tlschema();
addnav("Vessa's Gem Shop","runmodule.php?module=vessa");
	}
...
output("<form action='runmodule.php?module=vessa&op=gembuy2' method='POST'><input name='buy' id='buy'><input type='submit' class='button'
value='buy'></form>",true);
...
if ($session['user']['gold'] < ($buy * $gemcost)) {output("Vessa gives you the finger after you attempt to pay her less than her gems are worth.`n`n"); }
else{
$cost=($buy * $gemcost);
...
output("<form action='runmodule.php?module=vessa&op=gemsell2' method='POST'><input name='sell' id='sell'><input type='submit' class='button'
value='sell'></form>",true);
...
if ($session['user']['gems'] < $sell) {output("Vessa raises her fist at you knowing you do not have that many gems.`n`n"); }
else{
$cost=($sell * $gemsell);
