...
if ($newestplayer == $session['user']['acctid']) {
	$newtext = "`nYou're the newest person to be sucked into The Empty City and appear in this town.  As such, you wander around, gaping at the sights, and generally looking lost.";
	$newestname = $session['user']['name'];
} else {
	$newtext = "`n`2Wandering the main street is `&%s`2, looking completely lost.";
...
$basetext = array(
	"`@`c`b%s Square`b`cThe town of %s hustles and bustles.  No one really notices that you're standing there as they are too absorbed in their own problems.  ".
	"You see various shops and businesses along the main street.  There is a curious looking rock to one side.  ".
	"On every side the town is surrounded by sky scrapers, trees, monuments and colossal buildings.`n`n",$vname,$vname
	);
$origtexts = array(
	"text"=>$basetext,
	"clock"=>"The clock on the tower reads `^%s`@.`n",
	"title"=>array("%s Square", $vname),
	"talk"=>"`n`%`@Nearby some people talk:`n",
	"sayline"=>"says",
	"newest"=>$newtext,
	"newestplayer"=>$newestname,
	"newestid"=>$newestplayer,
	"gatenav"=>"Town Outskirts",
	"fightnav"=>"Combat Supplies District",
	"marketnav"=>"General Supplies District",
	"tavernnav"=>"Relaxation District",
	"infonav"=>"Info",
	"othernav"=>"Other",
	"section"=>"village",
	"innname"=>$iname,
...
addnav("L?List Wanderers","list.php");
...
if ($session['user']['superuser']&~SU_DOESNT_GIVE_GROTTO){
  addnav("X?`bSuperuser Penthouse`b","superuser.php");
}
