...
page_header();
output("`cWelcome to In Fear, a browser based, text based role playing game, based on the code of Legend of the Green Dragon, based on Seth Able's Legend of the Red Dragon.`n");
...
if (getsetting("homenewestplayer", 1)) {
	...
	if ($name != "") {
		output("`QThe newest runner in the City is: `&%s`0`n`n", $name);
	}
}

clearnav();
addnav("New to In Fear?");
addnav("Create a character","create.php");
addnav("Game Functions");
addnav("Forgotten Password","create.php?op=forgot");
addnav("List Wanderers","list.php");
addnav("Daily News", "news.php");
addnav("Other Info");
addnav("About In Fear and LoGD","about.php");
