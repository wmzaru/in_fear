...
while (list($key,$val)=each($order)){
	...
	case "2":
		 ...
		 * NOTICE
		 */
                //the code below is the page spilt
                rawoutput("<hr>");$impressum = getsetting("impressum", "");
		if ($impressum > "") {
			require_once("lib/nltoappon.php");
			output_notl("%s", nltoappon($impressum));
		}
		// line below is title info.
                output("Fear Mythos MMO`n");
                output("A MMO featuring the Fear Mythos`n`n");
                //Lines below hold game info
                output("What is the Fear Mythos? `@\"`2<a href='http://thefearmythos.com' target='_blank'>The Fear Mythos</a> is explained well enough in the Series Bible that I won't explain it here. if you came here without knowing the Mythos, I suggest you read it.", true);
                output("`n`n`@\"`2The Fear Mythos MMO project was started on January 20, 2013 in the <a href='http://theonlythingtofear.b1.jcink.com/index.php' target='_blank'>Fear Mythos Forums</a> by WereMagi.`n",true);
                output("`n`n`@\"`2The MMO is designed in tying all of the Mythos's Verses together into one game, to accomadate roleplayers and gamers alike whilst holding to the Fear's great power. The game is currently in beta as WereMagi slowly adds in more features and rewrites content to ensure a very Fear Mythos game.");
                output("`n`n`@\"`2The Fears themselves are opensource, but we did not create the Slender Man or the Rake. Characters that aren't featured in <a href='http://thefearmythos.wikia.com/wiki/Characters_free_to_use_in_Games' target='_blank'> this list</a> are used with permisson of their owners/authors.", true);
