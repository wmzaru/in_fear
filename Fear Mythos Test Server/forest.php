...
if ($op=="search"){
	...
	if ($session['user']['turns']<=0){
		...
		if (module_events("forest", getsetting("forestchance", 15)) != 0) {
			...
			if ($type=="slum"){
				$nlev++;
				output("`\$You head for the section of The Empty City you know to contain foes that you're a bit more comfortable with.`0`n");
			}
			if ($type=="thrill"){
				$plev++;
				output("`\$You head for the section of The Empty City which contains creatures of your nightmares, hoping to find one of them injured.`0`n");
			}
			$extrabuff = 0;
			if ($type=="suicide"){
				...
				output("`\$You head for the section of The Empty City which contains creatures of your nightmares, looking for the biggest and baddest ones there.`0`n");
			}
			$multi = 1;
